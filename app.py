...
os.get_environ['HUGGING_FACE_API']='enter api key here'
    ...
    if st.session_state.messages[-1]["role"] != "😇":
        with st.chat_message("😇"):
            with st.spinner("Thinking..."):
               ...
               ####use hugging face model here
                response="Use your rag chain model to get response here"
                if len(response)>0:
                    st.write(response)
                else :
